"""
Auto-detect (rule-based) spell import UI for the D&D Spellbook.

Two small dialogs:

* ``AddSpellSourceDialog`` - asks whether to add a spell manually or by pasting
  a block of text for auto-detection.
* ``SpellTextImportDialog`` - the paste box + disclaimer; parses the text into
  one or more ``ParsedObject`` results for review in the normal spell editor.
...
class AddSpellSourceDialog(ctk.CTkToplevel):
    """Ask how the user wants to create a new spell."""
    ...
    def __init__(self, parent):
        ...
        ctk.CTkLabel(
            container, text="How would you like to add this spell?",
            font=ctk.CTkFont(size=15, weight="bold"),
        ).pack(fill="x", pady=(0, 4))

        ctk.CTkLabel(
            container,
            text="You can fill in every field yourself, or paste a block of spell "
...
_PLACEHOLDER = """Paste one or more spells here, for example:
...
class SpellTextImportDialog(ctk.CTkToplevel):
    """Collect a block of text and parse it into one or more spell drafts."""
    ...
    def __init__(self, parent):
        ...
        self.title("Auto-Detect Spells From Text")
        ...
        ctk.CTkLabel(
            container, text="Paste spell text below",
            font=ctk.CTkFont(size=15, weight="bold"), anchor="w",
        ).pack(fill="x")

        ctk.CTkLabel(
            container,
            text="Multiple spells can be pasted at once - no numbering or separators "
                 "needed. Spellbook looks for a name followed by a line like "
                 "\"Level 3 Evocation\" or \"Evocation Cantrip\" to tell them apart.",
            font=ctk.CTkFont(size=11), text_color=theme.get_text_secondary(),
        ...
        ctk.CTkLabel(
            ...
            text="⚠  Auto-detection is rule-based and may not be fully accurate, "
                 "especially for irregularly formatted text. Every detected spell "
                 "opens in the editor for you to review and correct before it is saved.",
    ...
    def _preview(self):
        from text_import.spell_parser import split_blocks
        text = self._get_text()
        if not text:
        ...
        self._status.configure(
            text=f"Detected {n} spell block{'s' if n != 1 else ''}."
        )

    def _detect(self):
        from text_import.spell_parser import parse_spell_text
        text = self._get_text()
        if not text:
            messagebox.showwarning("No Text", "Please paste some spell text first.",
                                   parent=self)
            return
        ...
        try:
            ...
            messagebox.showerror(
                ...
                "Try pasting a single spell, or add it manually instead.",
                parent=self,
            )
            return
        ...
        if not parsed:
            messagebox.showwarning(
                "Nothing Detected",
                "Could not find a spell in that text.\n\n"
                "Make sure each spell has a name line followed by a line such as "
                "\"Level 2 Evocation\" or \"Evocation Cantrip\".",
                parent=self,
            )
            return
