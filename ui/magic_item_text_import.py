"""
Auto-detect (rule-based) magic item import UI for the D&D Spellbook.

Two small dialogs, mirroring ui/spell_text_import.py:

* ``AddMagicItemSourceDialog`` - asks whether to add an item manually or by
  pasting a block of text for auto-detection.
* ``MagicItemTextImportDialog`` - the paste box + disclaimer; parses the text
...
class AddMagicItemSourceDialog(ctk.CTkToplevel):
    """Ask how the user wants to create a new magic item."""
...
_PLACEHOLDER = """Paste one or more magic items here, for example:
...
class MagicItemTextImportDialog(ctk.CTkToplevel):
    """Collect a block of text and parse it into one or more magic item drafts."""
    ...
    def __init__(self, parent):
        ...
        self.title("Auto-Detect Magic Items From Text")
        ...
        ctk.CTkLabel(
            ...
            text="Multiple items can be pasted at once - no numbering or "
                 "separators needed. Spellbook looks for the standard "
        ...
        ctk.CTkLabel(
            ...
            text="⚠  Auto-detection is rule-based and may not be fully accurate, "
                 "especially for irregularly formatted text. Cost and weight "
    ...
    def _preview(self):
        from text_import.magic_item_parser import split_blocks
        text = self._get_text()
        if not text:
    ...
    def _detect(self):
        from text_import.magic_item_parser import parse_magic_item_text
        text = self._get_text()
        if not text:
            messagebox.showwarning("No Text", "Please paste some magic item text first.",
                                   parent=self)
            return

        try:
            parsed = parse_magic_item_text(text)
        ...
        if not parsed:
            messagebox.showwarning(
                "Nothing Detected",
                "Could not find a magic item in that text.\n\n"
                "Make sure each item has a name line followed by a line like "
