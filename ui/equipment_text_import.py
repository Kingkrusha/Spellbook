"""
Auto-detect (rule-based) equipment import UI for the D&D Spellbook.

Two small dialogs, mirroring ui/spell_text_import.py:

* ``AddEquipmentSourceDialog`` - asks whether to add an item manually or by
  pasting a block of text for auto-detection.
* ``EquipmentTextImportDialog`` - the paste box + disclaimer; parses the text
  into one or more ``ParsedObject`` results for review in the normal editor.
...
class AddEquipmentSourceDialog(ctk.CTkToplevel):
    """Ask how the user wants to create a new equipment item."""
    ...
    def __init__(self, parent):
        ...
        ctk.CTkLabel(
            container, text="How would you like to add this item?",
        ...
        ctk.CTkLabel(
            ...
            text="You can fill in every field yourself, or paste a block of "
                 "equipment text and let Spellbook fill in what it can for "
                 "you to review.",
...
_PLACEHOLDER = """Paste one or more items here, for example:
...
A versatile blade favored by knights and mercenaries.
"""


class EquipmentTextImportDialog(ctk.CTkToplevel):
    """Collect a block of text and parse it into one or more equipment drafts."""
    ...
    def __init__(self, parent):
        ...
        self.title("Auto-Detect Equipment From Text")
        ...
        ctk.CTkLabel(
            container, text="Paste equipment text below",
            font=ctk.CTkFont(size=15, weight="bold"), anchor="w",
        ).pack(fill="x")

        ctk.CTkLabel(
            container,
            text="Multiple items can be pasted at once - no numbering or "
                 "separators needed. Spellbook looks for a cost (\"15 gp\"), "
                 "a weight (\"3 lb\"), or a labelled Type/Cost/Weight line to "
                 "tell items apart.",
            font=ctk.CTkFont(size=11), text_color=theme.get_text_secondary(),
        ...
        ctk.CTkLabel(
            ...
            text="⚠  Auto-detection is rule-based and may not be fully accurate, "
                 "especially for irregularly formatted text - equipment varies "
                 "more in layout than spells do. Every detected item opens in "
                 "the editor for you to review and correct before it is saved.",
    ...
    def _preview(self):
        from text_import.equipment_parser import split_blocks
        text = self._get_text()
        if not text:
        ...
        self._status.configure(
            text=f"Detected {n} item{'s' if n != 1 else ''}."
        )

    def _detect(self):
        from text_import.equipment_parser import parse_equipment_text
        text = self._get_text()
        if not text:
            messagebox.showwarning("No Text", "Please paste some equipment text first.",
                                   parent=self)
            return

        try:
            parsed = parse_equipment_text(text)
        except Exception as exc:  # never let a parser hiccup kill the dialog
            messagebox.showerror(
                "Could Not Parse",
                f"Something went wrong reading that text:\n{exc}\n\n"
                "Try pasting a single item, or add it manually instead.",
                parent=self,
            )
            return
        ...
        if not parsed:
            messagebox.showwarning(
                "Nothing Detected",
                "Could not find an item in that text.\n\n"
                "Make sure each item has a name line, ideally followed by a "
                "cost or weight.",
                parent=self,
            )
            return
