...
class ImportDialog(ctk.CTkToplevel):
    ...
    def _create_widgets(self):
        ...
        info_frame.pack(fill="x", pady=(18, 0))
    ...
    def _import_json(self):
        ...
        link = bool(self.link_mentions_var.get())
        total_report = content_io.ImportReport()
        file_lines = []           # one line per file for the result dialog
        bad_files = []
        splash = ImportProgressSplash(self)
        try:
            for index, path in enumerate(file_paths):
                name = os.path.basename(path)
                try:
                    data = content_io.load_bundle_file(path)
                except ValueError as e:
                    bad_files.append(f"{name}: {e}")
                    continue
                except OSError as e:
                    bad_files.append(f"{name}: {e}")
                    continue

                def progress(text, fraction, _i=index, _n=len(file_paths), _name=name):
                    splash.update_progress(text, (_i + fraction) / _n, _i + 1, _n, "files")

                try:
                    report = content_io.import_bundle(
                        data, self.spell_manager, link_mentions=link, progress=progress,
                        defer_link_check=True)
                except Exception as e:                  # a bug must not lose the other files
                    bad_files.append(f"{name}: import failed ({e})")
                    continue
                total_report.merge(report)
                summary = "; ".join(report.summary_lines()) or "nothing new"
                file_lines.append(f"{name}: {summary}")
        finally:
            splash.destroy()
        # links between the files just imported only make sense to check once they are all in
        content_io.finalize_link_check(total_report)

        if total_report.imported or total_report.skipped or total_report.failed or bad_files:
            ImportResultDialog(self, total_report, file_lines, bad_files)
...
class ImportResultDialog(ctk.CTkToplevel):
    """Scrollable summary of an import: what was added, what was skipped and why."""

    def __init__(self, parent, report, file_lines, bad_files):
        super().__init__(parent)
        self.theme = get_theme_manager()
        self.title("Import Complete" if report.imported else "Import Finished")
        ...
        problems = report.problem_lines(limit=25) + [f"Skipped file {b}" for b in bad_files]
        ...
        if len(file_lines) > 1:
            ...
            lines += [f"  • {f}" for f in file_lines]
        if problems:
            lines.append("")
            lines.append(f"Notes ({len(problems)}):")
...
class ExportDialog(ctk.CTkToplevel):
    """Dialog for exporting the user's own content with type and source selection."""
    ...
    def _create_widgets(self):
        ...
        self._kind_by_label = {content_io.label(k): k for k in content_io.DISPLAY_ORDER}
        content_types = ["All"] + [content_io.label(k) for k in content_io.DISPLAY_ORDER]
    ...
    def _selected_kinds(self):
        """content_io kinds chosen in the dropdown (None = all of them)."""
        choice = self.content_type_var.get()
        if choice == "All":
            return None
        kind = self._kind_by_label.get(choice)
    ...
    def _update_sources(self):
        ...
        sources = content_io.unofficial_sources(kinds[0] if kinds else "all", self.spell_manager)

        self.source_combo.configure(values=["All Sources"] + sources)
        self.source_var.set("All Sources")
        self._update_info()

    def _get_export_counts(self) -> dict:
        """Get counts of items that will be exported, keyed by content_io kind."""
        import content_io

        source = self.source_var.get()
        source_filter = None if source == "All Sources" else source
        found = content_io.collect_export(self.spell_manager, self._selected_kinds(), source_filter)
        return {kind: len(objs) for kind, objs in found.items()}
    ...
    def _update_info(self):
        ...
        if not counts:
            ...
            for kind, count in counts.items():
                lines.append(f"  • {count} {content_io.label(kind).lower()}")
                total += count
            lines.append(f"\nTotal: {total} item(s)")
    ...
    def _export(self):
        ...
        if choice == "All":
            ...
            default_name = f"{choice.lower().replace(' ', '_')}_export.json"
        ...
        try:
            exported = content_io.export_to_file(
                file_path, self.spell_manager, self._selected_kinds(), source_filter)
            lines = [f"{n} {content_io.label(k).lower()}" for k, n in exported.items()]
            messagebox.showinfo(
                "Export Complete",
