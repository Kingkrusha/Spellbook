"""
Auto-detect (rule-based) lineage import UI for the D&D Spellbook.

Two small dialogs, mirroring ui/spell_text_import.py:

* ``AddLineageSourceDialog`` - asks whether to add a lineage manually or by
  pasting a block of text for auto-detection.
* ``LineageTextImportDialog`` - the paste box + disclaimer; parses the text
  into one or more ``ParsedObject`` results for review in the normal lineage
  editor.
...
class AddLineageSourceDialog(ctk.CTkToplevel):
    """Ask how the user wants to create a new lineage."""
    ...
    def __init__(self, parent):
        ...
        ctk.CTkLabel(
            container, text="How would you like to add this lineage?",
            font=ctk.CTkFont(size=15, weight="bold"),
        ).pack(fill="x", pady=(0, 4))

        ctk.CTkLabel(
            container,
            text="You can fill in every field yourself, or paste a block of lineage "
...
_PLACEHOLDER = """Paste one or more lineages here, for example:

Changeling
Creature Type: Fey
Size: Medium or Small
Speed: 30 feet
With ever-changing appearances, changelings reside in many societies
undetected...

Changeling Instincts. Thanks to your connection to the fey realm, you gain
proficiency in two of the following skills of your choice: Deception,
Insight, Intimidation, Performance, or Persuasion.

Shape-Shifter. As an action, you can shape-shift to change your appearance...
"""


class LineageTextImportDialog(ctk.CTkToplevel):
    """Collect a block of text and parse it into one or more lineage drafts."""
    ...
    def __init__(self, parent):
        ...
        self.title("Auto-Detect Lineages From Text")
        ...
        ctk.CTkLabel(
            container, text="Paste lineage text below",
            font=ctk.CTkFont(size=15, weight="bold"), anchor="w",
        ).pack(fill="x")

        ctk.CTkLabel(
            container,
            text="Multiple lineages can be pasted at once - no numbering or "
                 "separators needed. Spellbook looks for a name followed by "
                 "creature type / size / speed to tell them apart, and splits "
                 "traits from run-in headings like \"Darkvision. You have...\".",
            font=ctk.CTkFont(size=11), text_color=theme.get_text_secondary(),
        ...
        ctk.CTkLabel(
            ...
            text="⚠  Auto-detection is rule-based and may not be fully accurate, "
                 "especially for irregularly formatted text. Splitting traits from "
                 "the base description is a best guess. Every detected lineage "
                 "opens in the editor for you to review and correct before it is "
                 "saved.",
    ...
    def _preview(self):
        from text_import.lineage_parser import split_blocks
        text = self._get_text()
        if not text:
        ...
        self._status.configure(
            text=f"Detected {n} lineage block{'s' if n != 1 else ''}."
        )

    def _detect(self):
        from text_import.lineage_parser import parse_lineage_text
        text = self._get_text()
        if not text:
            messagebox.showwarning("No Text", "Please paste some lineage text first.",
                                   parent=self)
            return

        try:
            parsed = parse_lineage_text(text)
        except Exception as exc:  # never let a parser hiccup kill the dialog
            messagebox.showerror(
                "Could Not Parse",
                f"Something went wrong reading that text:\n{exc}\n\n"
                "Try pasting a single lineage, or add it manually instead.",
                parent=self,
            )
            return
        ...
        if not parsed:
            messagebox.showwarning(
                "Nothing Detected",
                "Could not find a lineage in that text.\n\n"
                "Make sure each lineage has a name line, ideally followed by "
                "its creature type, size, and speed.",
                parent=self,
            )
            return
